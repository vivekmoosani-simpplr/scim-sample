...
let scimDef = require("scimgateway/lib/scimdef-v2");
scimDef.Schemas.Resources.push({
  id: "urn:simpplr:scim:schemas:extension:b5a2c6a9-8cfb-4030-a90b-744befb0bc6c:2.0:User",
  name: "SimpplrUser-b5a2c6a9-8cfb-4030-a90b-744befb0bc6c",
  description: "SimpplrUser for Tenant b5a2c6a9-8cfb-4030-a90b-744befb0bc6c",
  attributes: [      
    {
...
scimDef.ResourceType.Resources.find(
  (res) => res.name === "User"
).schemaExtensions.push({
  schema:
    "urn:simpplr:scim:schemas:extension:b5a2c6a9-8cfb-4030-a90b-744befb0bc6c:2.0:User",
  required: false,
});
...
scimgateway.getUsers = async (baseEntity, getObj, attributes) => {
  ...
  const delta = arr.slice(
    getObj.startIndex - 1,
    getObj.startIndex - 1 + getObj.count
  ); // supporting paging "light"
...
scimgateway.createUser = async (baseEntity, userObj) => {
  ...
  if (userObj.userName)
    userObj.id = userObj.userName; // id set to userName or externalId
  else if (userObj.externalId) userObj.id = userObj.externalId;
  else
    throw new Error(
      `${action} error: missing mandatory userName or externalId`
    );
...
scimgateway.deleteUser = async (baseEntity, id) => {
  ...
  if (res.length !== 1)
    throw new Error(`${action} error: failed for user id=${id}`);
  const userObj = res[0];
...
scimgateway.modifyUser = async (baseEntity, id, attrObj) => {
  ...
  const res = users.find({ id: id });
  if (res.length === 0)
    throw new Error(`${action} error: user id=${id} - user does not exist`);
  if (res.length > 1)
    throw new Error(
      `${action} error: user id=${id} - user is not unique, more than one have been found`
    );
  const userObj = res[0];
    ...
    if (Array.isArray(attrObj[key])) {
      ...
      attrObj[key].forEach((el) => {
        if (el.operation === "delete") {
          ...
          let exists;
          if (el.value)
            exists = userObj[key].find(
              (e) => e.value && e.value === el.value && e.type === el.type
            ); // allowing same value on different type (type not mandatory)
        ...
        for (const sub in attrObj[key]) {
          ...
          if (sub === "attributes" && Array.isArray(attrObj[key][sub])) {
            attrObj[key][sub].forEach((element) => {
              ...
              if (arrSub.length === 2)
                userObj[arrSub[0]][arrSub[1]] = ""; // e.g. name.familyName
...
scimgateway.modifyGroup = async (baseEntity, id, attrObj) => {
  ...
  const res = groups.find({ id: id });
  if (res.length === 0)
    throw new Error(`${action} error: group id=${id} - group does not exist`);
  if (res.length > 1)
    throw new Error(
      `${action} error: group id=${id} - group is not unique, more than one have been found`
    );
  const groupObj = res[0];
  ...
  await attrObj.members.forEach(async (el) => {
    if (el.operation && el.operation === "delete") {
      ...
      if (el.value) {
        ...
        const usrs = await scimgateway.getUsers(
          baseEntity,
          getObj,
          "id,displayName"
        ); // check if user exist
        if (
          usrs &&
          usrs.Resources &&
          usrs.Resources.length === 1 &&
          usrs.Resources[0].id === el.value
        ) {
          const newMember = {
  ...
  if (usersNotExist.length > 0)
    throw new Error(
      `${action} error: failed for id=${
        groupObj.id
      } - includes none existing members: ${usersNotExist.toString()}`
    );
  return null;
};
